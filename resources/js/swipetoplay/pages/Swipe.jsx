...
const Swipe = () => {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [currentIndex, usuariosSwipe, obtenerUsuariosSwipe, obtenerConexion]);
            ...
            <div className='flex flex-col justify-around'>
              <div className='flex'>
                {currentIndex < usuariosSwipe.length && (
                  <motion.div className="flex flex-col md:w-[448px] tres:w-[350px] md:pb-36 pb-10 mx-auto text-white shadow-lg rounded-md overflow-hidden transform transition-transform cursor-pointer lg:bg-cover tres:bg-stretch bg-no-repeat bg-center"
                  style={{
                  ...
                  >
                    <div className="flex flex-col items-center p-4">
                      <img className="lg:w-36 lg:h-36  tres:w-20 tres:h-20 rounded-full mb-4" src={usuariosSwipe[currentIndex].imagen || imagenUser} alt="User" />
                      <h2 className="lg:text-2xl tres:text-md font-montserrat text-white">{usuariosSwipe[currentIndex].username}</h2>
                      <h3 className="lg:text-xl tres:text-sm font-Swipe text-white">{usuariosSwipe[currentIndex].likes}</h3>
                      <box-icon name='heart' type='solid' color='#d80707'></box-icon>
                    </div>
                    <div className="px-16 bg-opacity-20 bg-black  ">
                      <h3 className="lg:text-lg tres:text-sm  text-white font-montserrat my-1">Descripción:</h3>
                      <p className="lg:text-base tres:text-[12px] text-white text-Swipe mt-2">{getDescripcionCortada(usuariosSwipe[currentIndex].descripcion)}</p>
                    </div>
                    <div className="bg-opacity-20 bg-black flex flex-col items-center justify-center">
                      <h3 className="lg:text-base tres:text-sm text-white font-montserrat mt-1">Juego</h3><span className="lg:text-base tres:text-[12px] font-Swipe mb-1">{conexion.riotID ? conexion.riotID : 'cargando...'}</span>
                      <h3 className="lg:text-base tres:text-sm  text-white font-montserrat mt-1">Rango</h3><span className="lg:text-basetres:text-[12px] font-Swipe mb-1">{conexion.rango ? conexion.rango : 'cargando...'}</span>
                      <h3 className="lg:text-base tres:text-sm  text-white font-montserrat mt-1">Posición</h3><span className="lg:text-base tres:text-[12px] font-Swipe mb-1">{conexion.posicion ? conexion.posicion : 'cargando...'}</span>
                    </div>
                  </motion.div>
                )}
              </div>
              <div className='flex justify-center space-x-12 lg:mt-4 tres:mt-2'>
      ...
      {loading && (
        <div className="fixed inset-0 bg-black bg-opacity-25 flex justify-center items-center">
          <div className="bg-white p-4 rounded-lg shadow-lg flex items-center">
            ...
            <span className="font-medium text-gray-800">Realizando swipe...</span>
          </div>
        </div>
      )}
      <Footer />
